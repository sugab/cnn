...
l1 = c.ConvolutionLayer(h.random((16, 1, 5, 5)), np.ones((16, 1)), p=2, activation=c.ReLULayer)
l2 = c.PoolingLayer(2, 2)
l3 = c.ConvolutionLayer(h.random((32, 16, 3, 3)), np.ones((32, 1)), activation=c.ReLULayer)
l4 = c.PoolingLayer(2, 2)
l5 = c.ConvolutionLayer(h.random((32, 32, 3, 3)), np.ones((32, 1)), p=1, activation=c.ReLULayer)
...
network = c.CNN(model, c.SoftmaxLayer, data, lr=0.00002, momentum=0.9, dropout=0.8)
t, v = network.run(500, 25)

np.save('tubes_relu_2_t', t)
np.save('tubes_relu_2_v', v)
